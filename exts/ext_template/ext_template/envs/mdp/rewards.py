...
from ext_template.managers.manager_base import ManagerTermBase
from ext_template.managers.manager_term_cfg import RewardTermCfg
...
def joint_vel_limits(
    env: ManagerBasedRLEnv, soft_ratio: float = 1.0, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
...
# 惩罚那些我们不希望发生的接触，通过检测接触力是否超过一个预设的阈值，然后统计每个环境中超出阈值的接触点的数量来进行惩罚。返回的是违反的次数。这可以看作是一个更“硬性”的惩罚。
...
#  函数返回的惩罚值反映了接触力超出安全阈值的程度。如果接触力都低于阈值，则惩罚为零。如果某些接触力超过了阈值，惩罚值将与超出量成比例增加。
